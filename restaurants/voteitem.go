...
func GetResult(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	rows, err := db.Query("SELECT m.Id, m.Name, r.Name, r.Location, m.Price, m.Vote FROM menu m INNER JOIN restaurant r ON m.RestId = r.Id ORDER BY m.Vote DESC, r.Name ASC LIMIT 1")
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		return
	}
